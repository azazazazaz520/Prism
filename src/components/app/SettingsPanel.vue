...
import VendorList from '../vendors/VendorList.vue';
import SyncSetup from '../workspace/SyncSetup.vue';
import PromptEditor from '../plugins/PromptEditor.vue';
...
/** 错误码到用户提示的映射 */
const ERROR_MESSAGES: Record<UpdateCheckErrorCode, string> = {
  network_unreachable: '无法连接 GitHub，请检查网络',
...
async function checkUpdate() {
  ...
  try {
    // 请求静态更新清单（由 Rust 后端请求，不再直接访问 GitHub API）
    const manifest = await invoke<WindowsUpdateManifest>('check_update');

    // 将静态清单转换为 UpdateDialog 使用的 ReleaseInfo 格式
    const release: ReleaseInfo = {
      tag_name: `v${manifest.version}`,
      name: `Prism v${manifest.version}`,
      body: manifest.release_notes,
      html_url: manifest.download_url,
    ...
    try {
      const parsed: UpdateCheckErrorResponse = JSON.parse(String(e));
      errorMessage = ERROR_MESSAGES[parsed.error_code] || parsed.message;
    } catch {
      errorMessage = String(e) || errorMessage;
    }
    updateTip.value = errorMessage;
    _updateTipTimer = setTimeout(() => {
      updateTip.value = '';
    }, 3000);
  } finally {
